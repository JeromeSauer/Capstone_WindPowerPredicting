...
MODEL_PATH = 'lr-model.pickle'
DATA_PATH ='../data/GEFCom2014Data/Wind/raw_data_incl_features.csv', parse_dates=['TIMESTAMP']
#DATA_PATH = 'raw_data_incl_features.csv'
...
def make_prediction(day='', modelpath=MODEL_PATH, zone=-1):
    ...
    if int(zone) != -1:
        ...
        for zone in range(1,11):
            ...
            if zone==1:
                temp = [[h, x] for h,x in zip(range(0,24), temp)]
                temp.insert(0, [zone])
                pred = temp
            else:
                pred[0].append(zone)  
                for i in range(0,24) :
                    pred[i+1].append(temp[i]) 
            #pred.append(temp)
    return pred
